...
function isRemoteConfig(config: AiConfig) {
    return config.channelMode === "remote";
}

function aiApiUrl(config: AiConfig, path: string) {
    return isRemoteConfig(config) ? `/api/v1${path}` : buildApiUrl(config.baseUrl, path);
}
...
/** Remote proxy responses include credit changes; refresh the signed-in user afterwards. */
function refreshRemoteUser(config: AiConfig) {
    if (isRemoteConfig(config)) void useUserStore.getState().hydrateUser();
}

/** Remote proxy only speaks plain model names (channel selection happens server-side). */
function requestModelName(config: AiConfig) {
    return isRemoteConfig(config) ? modelOptionName(config.model) : config.model;
}
...
async function resolveAgnesImageInput(image: ReferenceImage) {
    ...
    if (!dataUrl) throw new Error(apiText("referenceImageUnavailable"));
    return dataUrl;
}
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    ...
    if (isAgnesImageConfig(requestConfig, requestConfig.model)) {
        return requestAgnesGeneration(requestConfig, prompt, n, undefined, [], options);
    }
    const quality = normalizeQuality(config.quality);
    const requestSize = resolveRequestSize(quality, config.size);
    const background = normalizeBackground(config.background);
    try {
        const response = await axios.post<ImageApiResponse>(
            ...
            {
                model: requestModelName(requestConfig),
                prompt: withSystemPrompt(requestConfig, prompt),
                n,
                ...(quality ? { quality } : {}),
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    ...
    if (isAgnesImageConfig(requestConfig, requestConfig.model)) {
        if (mask) throw new Error(apiText("agnesMaskUnsupported"));
        return requestAgnesGeneration(requestConfig, requestPrompt, n, undefined, references, options);
    }
    ...
    formData.set("model", requestModelName(requestConfig));
...
function parseChatStreamChunk(chunk: string, onDelta: (value: string) => void) {
    let deltaText = "";
...
/** The Go backend proxy only exposes /chat/completions for text, so remote mode streams through it. */
async function requestRemoteChatCompletion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void) {
    let buffer = "";
    let answer = "";
    ...
    try {
            ...
            },
            {
                ...
                onDownloadProgress: (event) => {
                    ...
                    for (const chunk of chunks) {
                        parseChatStreamChunk(chunk, (delta) => {
                            answer += delta;
                            onDelta(answer);
                        });
                    }
        ...
        if (typeof response.data === "object" && response.data && "code" in response.data && (response.data as { code?: number; msg?: string }).code !== 0) {
            throw new Error((response.data as { msg?: string }).msg || apiText("requestFailed"));
        }
        if (typeof response.data === "string") {
            let apiError = "";
            try {
                const payload = JSON.parse(response.data) as { code?: number; msg?: string };
                if (typeof payload.code === "number" && payload.code !== 0) apiError = payload.msg || apiText("requestFailed");
            } catch {
                // plain-text stream content, ignore
            }
            if (apiError) throw new Error(apiError);
        }
        if (buffer) {
            parseChatStreamChunk(buffer, (delta) => {
                answer += delta;
                onDelta(answer);
            });
        }
    ...
    refreshRemoteUser(config);
    return answer || apiText("noContent");
}
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    ...
    try {
        if (isRemoteConfig(requestConfig)) {
            const answer = await requestRemoteChatCompletion(requestConfig, messages, onDelta);
