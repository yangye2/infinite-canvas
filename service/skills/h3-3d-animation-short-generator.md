...
name: H3 3D动画短片生成器
description: |
  将一句故事创意转化为完整的风格化3D动画短片制作流程，依次完成项目简报、故事大纲、角色卡、无人物场景卡、标准镜头表、文本分镜或可选铅笔分镜、当前视频模型确认、单镜头生成、片段顺序编组、BGM匹配方案与最终复查。适用于需要角色一致、场景连续、节奏明确和音画配合的剧情动画短片、纪念短片、品牌故事与社交媒体内容。不适用于单张图片、简单修图、真人写实视频或单一独立镜头。
---

# 3D动画短片生成器

当用户想把一句话创意做成完整的故事优先动画短片流程时使用本 Skill。流程从创意开发到最终剪辑，所有主要产物都必须按生产顺序放到画布上，并在高成本或关键创意节点通过普通聊天列出明确选项让用户确认。

核心规则：**先故事、再资产；用户输入内容后立刻通过普通聊天确认画面尺寸/比例与总时长；产物按顺序上画布；所有需要用户确认的地方必须列出明确选项并等待确认；角色/场景卡后固定顺序为：六列标准镜头信息表（每秒指令+音频+空间锚点链+hook）→ 镜头表自检门 → 一份单一文本分镜文档（每镜一节，对齐半解说短剧分镜字段；用户主动开启可视化模式时再加铅笔图；用户标记需要重点迭代的镜时抽取成独立节点）→ 当前全局视频模型确认 + 分辨率确认 → 用当前全局视频模型生成单镜头片段 → 片段顺序编组、用户提供的 BGM（如有）与外部合成方案；规划的最终视频必须清除所有分镜痕迹**。

## 当前画布能力规则

- 下文“选项卡”统一表示在普通聊天中列出相同选项并等待用户选择，不表示调用结构化选择卡工具。
- 当前画布可以生成文本、图片、单镜头视频和朗读语音并对节点分组，但没有音乐生成、视频拼接或音轨混合工具。`generate_audio` 只用于旁白、口播或对白等实际朗读文字，不得用于生成 BGM。下文涉及 BGM 时只能使用用户提供的现有音频节点；用户未提供时，创建外部配乐说明并询问用户提供或在外部制作。全片拼接、BGM 混音或最终合成只能交付真实片段编组和外部处理方案，不得声称已经生成最终合成文件。

## 全局视觉风格锁

除非用户明确指定其他视觉风格，所有角色卡、场景卡、镜头表格、文本分镜、可选铅笔分镜、单镜头视频片段和外部最终合成方案都必须使用以下视觉风格：
...
- 一句话创意或粗略故事
- 目标产物：只要蓝图、只要资产、六列标准镜头信息表、单文本分镜文档（默认）+ 重点迭代镜抽取的独立节点 + 多宫格铅笔分镜（开启可视化模式时）、使用当前全局视频模型生成的单镜头视频片段，还是片段编组、用户提供的 BGM（如有）与外部最终合成方案
- 用户已说明的预估时长
- 用户已说明的画面尺寸 / 比例
- 视觉基调：默认温暖风格化 3D 动画
- 台词需求：有对白、旁白，还是无台词
...
只有用户同时选择画面尺寸/比例和总时长，或明确给出自定义值后，才进入下一步。把已确认的画面尺寸/比例和总时长写入项目简报，并贯穿用于镜头时长、转场连贯性、六列标准镜头信息表、单镜头分镜（默认文本，开可视化模式时含铅笔图）、单镜头视频片段、片段编组、BGM 匹配和外部最终合成设置。
...
7. 单镜头视频片段节点（用 Step 7 确认的当前全局视频模型渲染）
8. 按镜头顺序编组的视频片段
9. 用户提供的 BGM 音频节点（如有）与外部拼接、混音方案文本节点
...
警告用户：后续修改已锁定的角色设计可能需要重做镜头表、单镜头分镜、单镜头视频片段、片段编组和外部最终合成方案。
...
在角色卡和场景卡锁定后，输出标准化视频提示作为镜头信息表。本步为强制步骤，不可与分镜或视频生成互换。在画布使用 `create_text_node` 创建包含 Markdown 表格的文本节点，命名为 `标准镜头信息表` 或 `standard-shot-table`。
...
- **镜头描述**：景别、镜头运动、荷兰角设计、表演风格、音效、负向提示和当前全局视频模型需要的生成备注，并强制包含 `Per-Second Directives` 子节。子节必须按秒拆解为 `0–1s`、`1–2s`、`2–3s` 等指令；亚秒关键节拍用 `2.0–2.5s` 风格标记。每条每秒指令必须覆盖全部 5 个必填要素：
...
六项全过后，在 Markdown 表格文本节点顶部加 `shot-table self-check: passed` 戳，并展示用户选项卡：
...
任何一项不通过就不进入 Step 6。返回 Step 5，列出失败行，Markdown 表格修好并重跑自检通过后，才能再次展示分镜批准选项卡。

## STEP 6：单文本分镜文档（默认）+ 多宫格铅笔分镜图（opt-in）

Step 5.5 自检通过后，先展示分镜模式选项卡，再生成任何分镜产物：

- **单文本分镜文档（默认，推荐）** — 整个短片一份画布文本节点，所有分镜作为文档内章节。对齐半解说短剧分镜结构：每镜字段（标题/hook/场景/角色/空间锚点/连续性/表演备注）加 Pixar 的每面板四象限内容 + 可选 ASCII 布局。承载完整质控载荷，成本几乎为零。Step 7 已确认的当前全局视频模型直接读取作为该镜渲染参考。
...
- **回填**：用户满意后，把独立节点折叠回文档（占位换成最新内容），独立节点保留作为历史版本；只有用户明确要求时才删除。
...
## STEP 7：确认当前全局视频模型 + 单镜头视频片段

### 当前全局视频模型确认（任何片段渲染前的强制门）

任何片段渲染前必须先告知用户画布当前配置的全局视频模型，并通过普通聊天让用户确认继续使用，或先到视频设置中切换。确认结果写入项目简报，本项目后续片段沿用当前全局视频模型；用户后续修改全局视频模型时，继续生成前重新确认。

### 分辨率选项卡（当前全局视频模型确认后）

当前全局视频模型确认后，再按该模型实际支持的范围展示分辨率选项卡；默认使用画布当前视频设置，并始终允许用户自定义：
...
每行批准后调用当前全局视频模型生成对应独立视频片段。每条片段必须严格使用单文本分镜文档中匹配的章节（若该章节已抽取成独立节点则从独立节点取）、角色卡和场景卡。
...
单文本分镜文档（已抽取的镜从独立节点取）直接作为当前全局视频模型的权威参考。外层 prompt 同时强调包装与设计语言、电影感镜头、弹性表演、动作连续性、光位和镜头选择；需要文字/UI 或模型直接生成音频时，再加入对应意图。统一追加：`Pixar-inspired 3D cartoon rendering, C4D + Octane look, stylized Q-version proportions, warm SSS skin, designed-with-detail hair, strong character design language, cinematic lighting, elastic squash-and-stretch performance, clean motion, on-brand color palette`。

### 当前模型失败回退

1. 第一次重试：直接引用表格中 `参考锚点` 块，用更明确的动作、镜头和连续性描述强化 prompt 后重渲染。
2. 第二次重试：减少一个非关键道具或动作，必要时去掉参考图改为纯文本生成。
3. 第三次重试：把该镜缩到 ≤6s，把砍掉的秒拆到 Step 5 的新相邻行，重跑 Step 5.5 自检再重渲。
4. 同一镜连续 3 次失败：暂停并通过普通聊天询问用户选择放宽要求、跳过该镜并标记 `placeholder: missing clip`，或手动提供参考视频；不要声称已自动切换视频模型。

### 所有片段渲染完成后

按镜头顺序放到画布，归组为 `<title> shot clips`，展示用户选项卡：

- 批准片段并生成全片编组与外部合成方案（推荐）
- 使用当前全局视频模型重渲染选定片段
- 修复角色不一致
- 修复场景不一致
- 加强分镜清理
...
如果渲染片段与已批准的 `参考锚点` 漂移（门框落到错边、人物从错边离屏、光位翻转），用直接引用 `参考锚点` 块的强化提示重渲染；持续漂移时暂停并询问用户选择强化参考、放宽要求，或先到视频设置切换全局模型。用户切换后重新读取并确认全局配置；Agent 不得自动切换模型。不要在片段编组或外部拼接方案中静默混合修正版和未修正版。

## STEP 8：片段顺序编组、BGM 匹配与最终输出方案

所有单镜头片段批准后，用 `create_group` 按表格顺序编组真实片段。用户已提供 BGM 音频节点时，将其作为外部合成方案的真实音频来源；没有 BGM 时，创建匹配故事氛围的外部配乐说明并询问用户提供或在外部制作，不得调用 `generate_audio` 生成音乐。当前画布不得声称已经把片段和 BGM 合成为完整主视频。

编组、外部拼接与 BGM 规则：

- 严格按已批准表格的镜头顺序。
- 用户提供的 BGM 或外部配乐说明必须匹配已批准时间线的节奏、情绪弧、喜剧节拍、追逐节奏和结尾氛围。
- 在外部混音方案中标明对白、非语言反应和重要音效下需要 duck BGM 的时间段。
- 外部拼接方案默认保留原片段音频和音效（除非用户要求替换）。
- 不要用 `generate_audio` 生成整片或逐镜 BGM。
- 不要加字幕或文字（除非用户明确要求）。
- 外部最终视频要求必须保持干净动画视觉、无任何分镜痕迹，包括没有 `[char:…] [scene:…] [shot:…]` 双绑定标签。

然后展示用户选项卡：

- 批准片段编组、BGM 与外部合成方案（推荐）
- 更换用户提供的 BGM，或调整外部配乐说明
- 调整 BGM 外部混音方案
- 重渲染选定片段
...
- 音效/动效同步（对照 `音频与对白轨` 列）
- BGM 外部混音方案是否平衡
- 外部最终视频方案明确要求无分镜痕迹：没有面板边框、铅笔线、箭头、标签、手写备注、时间标记、姿态残影、分镜文字，**也没有双绑定标签**（`[char:…]`、`[scene:…]`、`[shot:…]`、`[dur:…]`、`[hook:…]`）
- 缺失或薄弱片段
- 任何可能需要重做的资产
...
- 单镜头视频片段归组为 `<title> shot clips`。
- 按顺序编组的片段、匹配 BGM 和外部合成方案在都有时归组为 `<title> final delivery`。

一轮生成产出 2 个或更多产物时，立刻归组并起一个清晰标题。渲染器位置未刷新导致归组失败时，请用户点击/拖动任一画布节点一次，再重试归组，必要时再进入下一个高成本环节。

## 用户确认纪律

每个需要用户确认的地方都通过普通聊天列出清晰、有限的选项并等待选择；不要用没有选项的散文提问代替。强制确认门：
...
- 单镜头视频片段渲染前：确认当前全局视频模型
- 单镜头视频片段渲染前：选视频分辨率
- 单镜头视频片段后
- 片段顺序编组、BGM 匹配和外部最终合成方案完成后
...
- 角色卡重做后，未来的镜头表、单文本分镜文档（含抽取的独立节点；可视化模式下还含铅笔分镜）、单镜头视频片段、片段编组和外部最终合成方案必须按精确角色名引用新的带标注角色卡。
- 场景卡重做后，未来的镜头表、单文本分镜文档（含抽取的独立节点；可视化模式下还含铅笔分镜）、单镜头视频片段、片段编组和外部最终合成方案必须按精确场景名引用新场景卡。
- 镜头表修订后，未来的分镜（文档+抽取节点）、单镜头视频片段和片段编组必须用新表。Step 5.5 的自检必须在分镜恢复前重跑。
- 单文本分镜文档中某章节修订后，对应单镜头视频片段必须用新章节内容。若该章节已抽取成独立节点，则独立节点为权威。
- 抽取的独立文本分镜节点修订后，用户满意时回填到文档（占位换成最新内容），独立节点保留作为历史版本；只有用户明确要求时才删除。
- 可视化模式下铅笔分镜重画后，对应视频片段仍以单文本分镜文档（或抽取的独立节点）为准；铅笔图仅供人眼参考，可独立重画而无需强制重渲视频。
- 单镜头视频片段重渲染后，片段编组、BGM 匹配和外部最终合成方案必须用新片段。用户修改当前全局视频模型后再重渲时，在项目简报记入新模型，并按新模型实际能力重新确认分辨率、时长和音频设置。
- 用户更换 BGM 后，外部最终合成方案必须使用新的真实音频来源。
