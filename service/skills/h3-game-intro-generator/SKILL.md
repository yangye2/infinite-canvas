...
- Before STEP 6, read the current global video model. If it is Minimax H3, use `references/h3-video-prompt-template.md` for the final H3 prompt. If it is not H3, ask the user to continue with the current model or switch to H3 in video settings; after any switch, read the configuration again. Fill the prompt from the approved confirmation image, player/game data, final UI copy, event timing, motion directions, and negative constraints.
...
## STEP 6: Refill the video prompt and generate with the current global model
After approval, read the current global video model. If it is Minimax H3, load `references/h3-video-prompt-template.md` and refill the final H3 prompt with confirmed style, character refs, player names, game title, UI text, event timing, motion directions, and negative constraints. If it is not H3, ask the user to continue with the current model or switch in video settings; never claim the Agent switched models. Generate only after the current global model is confirmed.
