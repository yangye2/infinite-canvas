...
name: H3 剪纸定格动画
description: |
  面向内容创作者、教育讲解者和社交视频编辑，用触感纸拼贴语言表现口播句、知识点、观点或抽象主题。用户提供短文案、故事节点或核心概念，也可补充画幅、时长、色调和音频需求。Skill 会提炼含义与视觉隐喻，制定制作方案和分镜，生成并确认半调纸拼贴静帧，再制作带纸片滑入、弹入、轻敲、压平和摩擦声的停格动画片段；多片段任务按顺序编组并提供拼接说明。默认保留拼贴音效，不主动添加 BGM、旁白和字幕。适合知识讲解、观点表达、故事配画和社交 B-roll，不适合真人口播广告、精确可编辑图层、复杂文字排版或仅输出提示词
...
- 默认不添加 BGM：可以询问用户是否需要 BGM；用户明确要求时使用其提供的现有音频节点，或交付外部配乐说明
- 默认不添加旁白口播：可以询问用户是否需要旁白讲解/口播，但只有用户明确要求时才写稿并使用 `generate_audio` 生成朗读语音；混入视频只能提供外部处理说明
- 默认不添加字幕：可以询问用户是否需要字幕，但只有用户明确要求时才创建字幕文案与时间点；烧录只能提供外部处理说明
- 视觉风格：高级编辑感半调纸拼贴
- 运动风格：触感停格组装，不使用慢速缩放、泛泛漂浮或平滑数字图层移动
- 视频生成始终使用当前全局视频模型；`MiniMax-H3` 是推荐选择，当前不是 H3 时询问用户继续使用当前模型，或先到视频设置切换后再继续
...
6. 提供外部拼接说明时，如果原片段音轨是拼贴音效，默认要求保留这些原始音轨，不要默认丢弃音频。
7. 只有当用户要求静音、视频原生音频中出现不需要的人声/音乐，或用户要求另行混入音乐/旁白时，才重生成片段或提供外部移除、替换与混音说明。只有用户明确要求时才创建字幕文案与时间点；不得声称当前画布已经烧录字幕。
...
- 如果生成片段中有可用拼贴音效，在外部拼接说明中要求保留它
- 如果生成片段出现不需要的人声、音乐、假 UI 音或嘈杂环境声，根据用户目标重生成片段，或提供外部移除音轨说明
- 保持镜头视觉稳定；除非用户要求，不增加运镜
- 保持拼贴材质语言、最终构图和已建立批次风格
- 修订后续片段时，可参考已通过的最佳前序片段风格和节奏

生成前读取并展示当前全局视频模型。`MiniMax-H3` 是推荐选择；当前不是 H3 时询问用户继续使用当前模型，或先到视频设置切换。用户明确指定其他模型时也必须由用户先在设置中切换，Agent 不得自动选择或替换模型。
...
只要隐喻清楚、最终构图仍像已批准静帧，微小细节漂移可以接受。若出现严重漂移、文字、假 UI、开场颜色不匹配、未经允许的牛皮纸底、过脏过皱纸质、丢失停格组装、不需要的 BGM 或不需要的旁白，应根据问题发生阶段重生成；若必须后期处理，向用户说明并提供处理建议。

## STEP 8：可选音频、编组与交付

如果用户要求多片段故事视频，使用 `create_group` 按确认的分镜顺序编组已批准片段，并提供外部拼接时间线，默认要求保留每段原始拼贴音效。

只有当用户明确要求旁白或口播时，才使用 `generate_audio` 朗读实际文字。用户要求独立 BGM 时，使用其提供的现有音频节点，或创建外部配乐说明；不得用 `generate_audio` 生成音乐。字幕只提供文案与时间点。需要混音或烧录时，提供外部处理说明，不声称当前画布已经完成混入或烧录。

每个完成项交付：

- 工具实际返回的 B-roll 视频节点；多片段时同时提供分组节点和拼接说明
- 相关已批准静帧节点
...
- 如果视频片段的原生音频缺少拼贴音效但画面可用，询问用户保留、重生成更强音效方向，或输出外部同步音效方案。
- 如果视频片段的原生音频出现不需要的 BGM 或旁白，重生成片段；当前画布无法直接移除已有音轨时必须如实说明。
