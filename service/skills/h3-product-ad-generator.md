...
- 当前可用能力是当前全局视频模型真实支持的原生音频、`generate_audio` 朗读语音、文本节点和节点编组；`generate_audio` 不能生成 BGM 或器乐。音乐生成、音频节拍检测、音乐裁切、替换视频音轨和最终混音合成不能在当前画布直接执行。
- 下文所有卡点、裁切、替换音轨和合成要求都必须保留为创作目标，但实际执行方式是依据用户提供或确认的时间点制定外部剪辑方案；不得声称当前画布已经检测、裁切、替换音轨或生成最终合成文件。
...
7. **视频模型策略**
   - Agent 只能使用画布当前全局视频模型，生成前通过 `get_generation_config` 读取并展示真实模型。
   - MiniMax-H3 仍是本 Skill 的推荐模型，因为它通常更容易生成干净的 Apple 感产品近景、留白和高级产品片镜头语言；如果当前全局模型不是 H3，询问用户继续使用当前模型，或先到视频设置切换为 H3。
   - 用户明确指定 Seedance 或其他模型时，提示用户先在视频设置中切换；切换后重新读取全局配置再继续。Agent 不得自动选择或切换模型。

启动问询是强制 Gate。即使用户已经上传了商品素材，也只能跳过“请上传素材”这一步；不能跳过其余启动问询。agent 仍必须在进入产品分析、锚定图、分镜或视频生成前，一次性确认商品款式情况、目标时长、画幅比例、苹果风模板和画面文案方式。视频模型不是启动问询问题：读取并展示当前全局视频模型；若不是推荐的 MiniMax-H3，询问用户继续使用当前模型或先到视频设置切换。如果用户说“你看着办 / 直接做”，agent 可以为其余字段选择推荐默认值，但仍必须把采用的选择作为已完成的启动问询结果展示出来，再继续下一步。
...
- 默认使用当前全局视频模型生成一条全画幅产品片，而不是逐镜生成多个首帧视频，除非用户明确要求其他构造方式。
- 当前全局视频模型支持原生音频时默认开启，除非用户明确要求无声。音乐提示词使用 Apple 风科技配乐方向，让画面节奏和声音一起生成；不支持原生音频时如实说明并转为外部配乐方案。
...
- 镜间连贯硬约束。
- 当前全局视频模型的原生音频能力、开关和音乐提示词；模型支持时默认开启，除非用户明确要求无声。
- 禁止传原商品图。

## 步骤 9：科技配乐

当前全局视频模型支持原生音频时，先把下面的 Apple 风科技配乐方向写入视频提示词并开启原生音频。如果模型不支持、直出的音乐不好听、太吵、太弱、不同步，或用户要求重新配乐，询问用户提供现有音乐音频节点，或创建外部配乐提示与替换音轨方案；不得调用 `generate_audio` 生成器乐，也不得声称当前画布已经替换视频音轨。`generate_audio` 只在用户明确需要旁白、口播或对白时使用。
...
最终成片前，必须根据用户提供或确认的音乐时间信息制定卡点剪辑与外部合成方案，而不是简单建议把音乐铺上去。

音乐时间信息：

- 使用用户提供或确认的 BPM、强拍、能量段、单音卡点和时间戳；没有这些信息时先询问，不得声称已经自动检测。
- 根据已确认信息选择连续有声、能量合适、适合产品展示的 8-12 秒候选片段。
- 不要盲目裁前 10 秒。
- 优先选择有明确进入点、后半段不静音、不塌陷、适合结尾稳定停留的片段。

外部音轨处理方案：

- 保留视频画面，建议裁取连续有声音乐片段并替换视频音轨，明确标注建议时间范围。
- 需要只保留新音乐时，在方案中注明原视频音轨静音。
- 音量必须克制，不做响度最大化、不做夸张限制器、不把音乐分贝拉得太大；但也不要压得过低，pluck、节奏点和产品展示需要清楚可听。
- 不做过长淡出；只在必要时做 0.1-0.3 秒轻微尾部淡出。
- 将长静音或听感掉音列为外部合成后的必检项；当前画布未执行合成时不得声称已经检测通过。
...
- 当前实际生成的视频、用户提供的音乐或生成的朗读语音（如有）和画布分组。
- 时长、比例和语言。
- 简短创意总结。
- 使用的文案、原生音频或外部独立音乐方案。
- 是否提供了卡点剪辑与外部音轨处理方案。
...
- 音乐太差：重生成当前模型支持的原生音频，或询问用户提供现有音乐并给出外部裁切方案；不得用 `generate_audio` 生成配乐。
- 成片后半段没声音：重生成带原生音频的视频；若改用用户提供的独立音乐，则请用户确认连续有声时间段并更新外部替换音轨方案。
