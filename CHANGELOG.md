...
+ [新增] 前端升级至 v0.16.0：多渠道管理、Gemini 与模型调用脚本、Agent 面板与 Skill、提示词来源与界面国际化。
+ [新增] 保留 Go 后端管理：登录注册、Linux.do OAuth、用户与算力点、提示词/素材管理、渠道与算力点配置。
+ [新增] 配置中心支持后台渠道与本地渠道双模式，后台渠道请求经站点代理并按算力点计费。
+ [新增] 本地渠道支持 Agnes 调用格式（生图直连 Agnes 接口）。
